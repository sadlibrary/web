...
const currentLibraryInput = document.getElementById(
  "add-file-current-library-input"
);
...
Array.from(LibListItems).forEach((item) => {
  item.addEventListener("click", function () {
    ...
    console.log(
      "before: " + currentLibraryInput.name + " " + currentLibraryInput.value
    );
    currentLibraryInput.value =
      this.getElementsByClassName("library-name-text")[0].innerHTML;
    console.log(
      "after: " + currentLibraryInput.name + " " + currentLibraryInput.value
    );
...
window.onload = function () {
  ...
  Array.from(LibListItems).forEach((item) => {
    if (
      item.getElementsByClassName("library-name-text")[0].innerHTML === element
    ) {
      try {
        mainSection.classList.remove("d-none");
      } catch (e) {}
